...
function NegativeCompleteLogLikelihood(P,T,choices_index,InitalP)
    #ll,alpha=ForwardPass(P,T,choices,InitalP)
    pi,PFwdState,PBackState,Pstate,_,_=ProbabilityState(P,T,choices_index,InitalP)
    Ntrials=length(choices_index)
    ecll=0
    for itrial in 1:Ntrials
        ecll+=sum(Pstate[itrial,:]*.log.(P[itrial,:,choices_index[itrial]]+epsilon) )
    end
...
function ProbabilityState(P,T,choices_index,InitalP)
    """
    ...
    choices_index the list of index outputs it is an integer 1,2.. NpossibleOutputs. Dimension: 1 x Ntrials
    ...
    ll,PFwdState=ForwardPass(P,T,choices_index,InitalP)
    ...
    for itrial in 1:Ntrials
        ...
        for istate in 1:Nstates
            if itrial==1
                ...
                PBackState[i,istate]=sum(  [ T[istate,k]*P[i+1,k,choices_index[i+1]]*PBackState[i+1,k] for k in 1:Nstates]   )
            end
        end
    ...
    for itrial in 1:Ntrials-1
        ...
        for i in 1:Nstates
            for j in 1:Nstates
                norm=norm+PFwdState[itrial,i]*T[i,j]*P[itrial+1,j,choices_index[itrial+1]]*PBackState[itrial+1,j]
            end
        end

        for i in 1:Nstates
            for j in 1:Nstates
                xi[itrial,i,j]=(PFwdState[itrial,i]*T[i,j]*P[itrial+1,j,choices_index[itrial+1]]*PBackState[itrial+1,j])/norm
            end
        end
    end
...
function MaximizeEmissionProbabilities(stim,delays,idelays,choices,past_choices,past_rewards,T,InitialP,args,x,lower,upper,consts=0,y=0)
        ...
        function MaxEmission(z)
            if z!=z
                ...
                ll=NegativeCompleteLogLikelihood(P,T,choices,InitialP)
                #println("ll: ",ll)
                if ll !=ll
